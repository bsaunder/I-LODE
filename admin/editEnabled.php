...
switch($func){
	case "set":
		$sql = "UPDATE objects SET enabled = $R1 WHERE id = $oid";
		$results = mysql_query($sql) or die("Couldnt Set Logging.");
		echo "<font size=3><b>Status Set.</b></font><br><br>";
		if($R1 == 0){
			echo "Object Was Enabled<br><br>";
		} else {
			echo "Object Was Disabled<br><br>";
		}
		break;
	default:
		...
		<form method=POST action=editEnabled.php>
		  <tr>
  ...
  <input type=radio value=1 name=R1>Disable
		...
		<input type=hidden name=func value=set>
		  <p align=center><input type=submit value=\"Set Status\" name=B1></p>
		</form>
		";
		break;
}
